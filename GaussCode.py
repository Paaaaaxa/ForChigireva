...
for i in range(N):
    ...
    m = MatrixA[i:N, i]  # Это большой прикол, но похоже, что это именно ссылка на объект
    MasCopyA = np.copy(MatrixA[iMaxNumber[0] + i])  # это глубокая копия и два независимых массива
    MasCopyB = np.copy(MatrixB[iMaxNumber[0] + i])
    MatrixA[iMaxNumber[0] + i] = MatrixA[i]
    MatrixB[iMaxNumber[0] + i] = MatrixB[i]
    MatrixA[i] = MasCopyA
    MatrixB[i] = MasCopyB
    MatrixA[i], MatrixB[i][0] = MatrixA[i] / MatrixA[i][i], MatrixB[i][0] / MatrixA[i][i]

    for j in range(i + 1, N):
        MatrixB[j][0] = MatrixB[j][0] - MatrixB[i][0] * MatrixA[j][i]
        MatrixA[j][i:N] = MatrixA[j][i:N] - MatrixA[i][i:N] * MatrixA[j][
            i]  # вычитаем строки срезы можно использовать и так: m = MatrixA[2,0:2]
...
for i in reversed(range(N)):
    ...
    o = MatrixB[0:i, 0]
    MatrixB[0:i, 0] = MatrixB[0:i, 0] - current.transpose()
    MatrixA[0:i, i] = 0.0
    print(MatrixA)  # печать на каждом шаге
    print(MatrixB)
